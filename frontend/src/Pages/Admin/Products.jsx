...
const Products = () => {
  ...
  const setting = useSelector((state) => (state.setting.noOfRow))
  const { setLoader2, fetchProduct } = useContext(context)
  ...
  useEffect(() => {
    if (setting && setting.noOfRow) {
      setItemOnPerPage(setting.noOfRow)
    }
  }, [setting])
  useEffect(() => {
    getSearchProduct()
  }, [allProduct, search, onlyInStock, itemOnPerPage, searchType])
  useEffect(() => {
    const items = products.slice((currentPage - 1) * itemOnPerPage, currentPage * itemOnPerPage)
    setProductOnPage(items)
        ...
        <div>
          <div className='top flex items-center justify-between'>
            ...
            <div className='top2'>
              ...
              <span className='totalproducts'>Total Products: {products.length}</span>
            </div>
          </div>
