...
const Product = (props) => {
    ...
    const [itemOnPerPage,setItemOnPerPage]=useState(60)
    ...
    useEffect(()=>{
      ...
      useEffect(()=>{
        if(props.setting && props.setting.productPageItemNo){
          setItemOnPerPage(props.setting.productPageItemNo)
        }
      },[props.setting])
    return (
    <div>
       ...
       <div className={`${(filterProduct.length===0)?'hidden':'flex'} flex-wrap items-center justify-evenly w-full`} style={{minHeight:'60vh'}} >
        ...
        <div><Footer setting={props.setting}/></div>
       </div>
        </div>
    </div>
    </div>
