    ...
    <div id="header" className={` ${(props.path==='/products')?'fixed top-0 left-0 ':''} ${(props.path.includes('/admin'))?'hidden':''} z-50 w-full bg-white top-0 h-36`}>
      ...
      <div className="flex items-center justify-between h-20 sm:px-8 px-2 border-b border-main-800">
        ...
        <form className="sm:hidden flex relative px-4 py-2 border-b border-main-800" onSubmit={(e)=>{submitSearch(e)}}>
          ...
          {searchItem.map((item,i)=>{
            if(i<6){
              return <span key={i} onClick={()=>{handleClickOnItem(item._id)}} className="flex bg-white border-y py-2 px-3 cursor-pointer w-full overflow-hidden whitespace-nowrap hover:bg-hover-50"><img className="mr-3" src={`${(item.thumbnail!=='')?`${BACKEND_URL}/Images/${item.thumbnail}`:"/Images/profile.jpg"}`} style={{width:'30px',height:'30px'}} alt="" />{item.name}</span>
            }
          })}
          </div>
          </div>
