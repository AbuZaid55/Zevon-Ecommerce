...
const Product = (prop) => {
    ...
    const filterAllProducts = ()=>{
        ...
        if(filterallproduct!=='' && filterallproduct!==false){
            setTotalPage(Math.ceil(filterallproduct.length/5))
    ...
    useEffect(()=>{
        const filterproduct = allFilterProduct.slice((currentPage-1)*30,(currentPage-1)*5+30)
  ...
  return (
    <div>
        <div className='fixed top-0 left-0 z-50 w-full'><Header user={prop.user}/></div>
        <div className=' mt-44 sm:mt-36 w-full flex fixed overflow-y-scroll'>
        <aside className={` bg-gray-200 ${(showFilter)?'left-0':' -left-60 '}  z-20 md:static h-full md:h-auto w-1/5 transition-all`} style={{minWidth:"230px",maxWidth:"300px"}}>
                ...
                <div className={`${(filterSubCategory.length===0 && filterPrice===maxPrice && rating===0)?'hidden':''}`}>
                    ...
                    <div className='p-2'>
                        {filterSubCategory.map((subCategory,i)=>{
                            return <span key={i} className='bg-white rounded-full px-3 py-1 block mt-1'>{subCategory}</span>
                        })}
